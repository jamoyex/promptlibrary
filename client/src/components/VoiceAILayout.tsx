...
import { ChevronDown, Wand2, FolderOpen, Library, Mic } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function VoiceAILayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#f3f7fb] flex flex-col">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center gap-2 text-sm font-semibold text-gray-800 px-2 hover:bg-gray-100 h-8">
                  <Mic className="h-4 w-4 text-[#4698d8]" />
                  Voice AI <ChevronDown className="w-4 h-4 text-gray-500" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="w-48">
                <Link href="/voiceai">
                  <DropdownMenuItem className="cursor-pointer gap-2">
                    <FolderOpen className="h-4 w-4 text-gray-500" /> Your Voice Bots
                  </DropdownMenuItem>
                </Link>
      ...
      <CreateVoiceModal open={createModalOpen} onOpenChange={setCreateModalOpen} />
